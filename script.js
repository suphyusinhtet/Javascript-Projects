...
const updateGame = (p1,p2,gameState) => {
  ...
  if (p1.health <= 0 || p2.health <= 0) {
    game.isOver = true; //important key
...
class Player {
  ...
  strike (player, enemy, attackDmg) {
    ...
    // let GetRandomdamage = () => { return Math.ceil(Math.random()*10) }
    // const damageAmount = GetRandomdamage()
  ...
  heal (player) {
    if(game.isOver == false){
      ...
      // let GetRandomhealAmount = () => { return Math.ceil(Math.random()*5)}
      // const hpAmount = GetRandomhealAmount()
...
class Game {
  ...
  declareWinner(isOver,p1, p2) {
    ...
    let message;
    // If isOver is true AND p1 health is <= 0 then update message variable  to 'p2 WINS!'
    if (isOver == true && p1.health <= 0) {
      message = `${p2.name} WINS!`;
    }
    // Else if isOver is true AND p2 health is <= 0 then update message variable  to 'p1 WINS!'
...
const player1 = new Player('Su',100,40)
// console.log(player1.attackDmg)
const player2 = new Player('Phyu',100,40)
